...
## Разработка и вклад
- Ветки, пулл-реквесты и issue — приветствуются.
- Стиль кода: nullable enabled, асинхронные методы возвращают `Task`/`Task<T>`, использование `RunWingetAsync` для взаимодействия с внешними инструментами.

## Лицензия
Добавьте нужную лицензию в корень репозитория (`LICENSE`) — пока проект предоставляется без явной лицензии.

---

Файлы для просмотра:
- Основной код: `VC++/MainWindow.xaml.cs`
- Проект: `VC++/VC++.csproj`

Если нужно — могу сгенерировать пример `CONTRIBUTING.md` или добавить опцию сохранения полных логов в файл.